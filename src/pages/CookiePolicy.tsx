...
import { Settings, Calendar, Info, CheckCircle2 } from 'lucide-react';
...
const CookiePolicy: React.FC = () => {
  ...
  useEffect(() => {
    async function loadPage() {
      try {
        const data = await getPage('cookie-policy');
        setPage(data);
        ...
        console.error('Failed to load cookie policy page:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (!page) {
    return (
      <div className="relative pt-32 pb-24 px-4 sm:px-6 md:px-12 lg:px-24 text-center">
        <SEO title="Cookie Policy | AeroSky" description="AeroSky Cookie Policy" />
        <div className="text-sky-200/50 text-sm">Cookie Policy is currently unavailable.</div>
      </div>
    );
  }

  const TAKEAWAYS = [
    { text: "Essential Only: Cookies are only used for session security and site speed." },
    { text: "No Cross-Site Ads: We do not run third-party advertising trackers." },
    { text: "Performance Opt-In: Optional analytical telemetry via GTM (Google Tag Manager) and Clarity." },
    { text: "Browser Control: Easily toggle or clear cookies in your browser settings." }
  ];
  ...
  return (
    <div className="relative min-h-screen pt-28 pb-20 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-1/4 left-1/4 -translate-x-1/2 w-[600px] h-[600px] bg-sky-500/[0.02] rounded-full blur-[120px]" />
        <div className="absolute bottom-1/4 right-1/4 translate-x-1/2 w-[500px] h-[500px] bg-emerald-500/[0.015] rounded-full blur-[100px]" />
      </div>

      <SEO
        title={page.seoTitle || `${page.title} | AeroSky`}
        description={page.seoDescription || "AeroSky Cookie Policy"}
      />
      <Schema
        type="BreadcrumbList"
        data={{
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-12 lg:px-24">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-6">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-sky-500/20 bg-sky-500/[0.04] text-[10px] font-mono font-bold tracking-widest text-sky-400 uppercase">
            <Settings size={12} /> Optimization Cookies
          </span>
          <span className="text-[10px] font-mono text-sky-200/30 uppercase tracking-widest">
            Document ID: ASK-POL-COOK
          </span>
          ...
          <aside className="lg:col-span-1 space-y-8 lg:sticky lg:top-28">
                  ...
                  {headings.map((h) => (
                    <button
                      ...
                      className={`text-left text-xs font-medium transition-all py-0.5 leading-snug cursor-pointer ${
                        activeId === h.id
                          ? 'text-sky-400 translate-x-1 font-semibold'
            ...
            <div className="glass p-5 rounded-2xl border border-white/[0.05] bg-white/[0.01]">
              <h3 className="text-xs font-bold text-white mb-3 flex items-center gap-1.5">
                <Info size={14} className="text-sky-400" /> Key Takeaways
              </h3>

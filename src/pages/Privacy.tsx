...
import { Shield, Calendar, Info, CheckCircle2 } from 'lucide-react';
...
const Privacy: React.FC = () => {
  ...
  const TAKEAWAYS = [
    { text: "No Sale of SDR Data: Your receiver location data is never sold to third parties." },
    { text: "Indian Data Residency: Airspace intelligence data is processed securely on Indian servers." },
    { text: "Minimal Personal Info: We only collect name and email for AeroCaptains onboarding." },
    { text: "Opt-Out Anytime: Disable your ground station telemetry feed at your convenience." }
  ];
  ...
  return (
    <div className="relative min-h-screen pt-28 pb-20 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-1/4 left-1/4 -translate-x-1/2 w-[600px] h-[600px] bg-emerald-500/[0.02] rounded-full blur-[120px]" />
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-12 lg:px-24">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-6">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-emerald-500/20 bg-emerald-500/[0.04] text-[10px] font-mono font-bold tracking-widest text-emerald-400 uppercase">
            <Shield size={12} /> Compliance & Safety
          </span>
          <span className="text-[10px] font-mono text-sky-200/30 uppercase tracking-widest">
            Document ID: ASK-POL-PRIV
          </span>
          ...
          <aside className="lg:col-span-1 space-y-8 lg:sticky lg:top-28">
                  ...
                  {headings.map((h) => (
                    <button
                      ...
                      className={`text-left text-xs font-medium transition-all py-0.5 leading-snug cursor-pointer ${
                        activeId === h.id
                          ? 'text-emerald-400 translate-x-1 font-semibold'
            ...
            <div className="glass p-5 rounded-2xl border border-white/[0.05] bg-white/[0.01]">
              <h3 className="text-xs font-bold text-white mb-3 flex items-center gap-1.5">
                <Info size={14} className="text-emerald-400" /> Key Takeaways
              </h3>

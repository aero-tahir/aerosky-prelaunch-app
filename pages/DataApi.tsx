...
const DataApi: React.FC = () => {
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-sky-950 pt-20 px-6 md:px-12 pb-20 transition-colors duration-300">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-slate-900 dark:text-white mb-4">Build with AeroSky API</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-20">
          ...
          <div className="space-y-8">
            <div className="glass-panel p-6 rounded-xl border-l-4 border-l-yellow-400">
              <h3 className="text-xl font-bold mb-2 font-mono text-slate-900 dark:text-white">GET /flights/:id</h3>
              <p className="text-slate-600 dark:text-gray-400 mb-4">Returns real-time telemetry, aircraft details, and route prediction for a specific flight.</p>
              <div className="flex gap-2 text-xs font-mono text-slate-500 dark:text-gray-500">
            ...
            <div className="glass-panel p-6 rounded-xl border-l-4 border-l-green-400">
              <h3 className="text-xl font-bold mb-2 font-mono text-slate-900 dark:text-white">WSS /stream/area</h3>
              <p className="text-slate-600 dark:text-gray-400 mb-4">Subscribe to a bounding box for ultra-low latency updates (200ms).</p>
            </div>
          </div>
    ...
    <ul className="space-y-4 mb-8 flex-1">
      {features.map((f, i) => (
        <li key={i} className="flex items-center gap-2 text-sm font-medium">
          {active ? <Shield size={16} className="text-black" /> : <Shield size={16} className="text-slate-400 dark:text-gray-500" />}
          {f}
        </li>
      ))}
    </ul>
